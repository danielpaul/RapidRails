...
class LandingPage::HeaderComponent < ApplicationComponent
  ...
  def mobile_menu
    div class: "lg:hidden", style: "display:none", aria_modal: "true", role: "dialog", x_show: "dropdownOpen" do
      ...
      div(class: "fixed inset-y-0 right-0 z-20 w-full overflow-y-auto bg-white dark:bg-body-dark px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-neutral-900/10") do
        ...
        div(class: "mt-6 flow-root") do
          div(class: "divide-neutral-500/10 -my-6 divide-y") do
  ...
  def mobile_menu_link_classes
    "-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-neutral-900 dark:text-white hover:bg-neutral-50 dark:hover:bg-card-dark hover:text-primary-500"
  end

  def mobile_menu_open_button
    div(class: "flex lg:hidden") do
      button(
        class: "inline-flex items-center justify-center rounded-md text-neutral-700 dark:text-neutral-300 -m-2.5 p-2.5",
        type: "button",
  ...
  def mobile_menu_close_button
    button(
      class: "rounded-md text-neutral-700 dark:text-neutral-300 -m-2.5 p-2.5",
      type: "button",
      "@click": "dropdownOpen = !dropdownOpen"
    ) do
